...
static void rs_solve_key_equation(gf2_poly_t *sigma, gf2_poly_t *omega, const gf2_poly_t *a, const gf2_poly_t *b)
{
	...
	CREATE_GF2_POLY(y, maxdegree); GF2_POLY_COEFF(y, 0) = 1;
	...
	while (!gf2_poly_is_zero(n) && gf2_poly_get_real_degree(n) >= gf2_poly_get_real_degree(y)) {
		...
		gf2_poly_copy(x, y); gf2_poly_copy(y, z);  // x = y; y = z;
		gf2_poly_copy(m, n); gf2_poly_copy(n, r);  // m = n; n = r;
	}

	// divide both y and n by y[0];
	CREATE_GF2_POLY(h, 0); GF2_POLY_COEFF(h, 0) = GF2_POLY_COEFF(y, 0);
...
int rs_fix_errors(gf2_poly_t *R, int error_words)
{
	...
	CREATE_GF2_POLY(z, error_words); GF2_POLY_COEFF(z, error_words) = 1; // z: x^7
	CREATE_GF2_POLY(sigma, error_words);
	CREATE_GF2_POLY(omega, error_words);
	rs_solve_key_equation(sigma, omega, z, S);

	CREATE_GF2_POLY(x, 1); GF2_POLY_COEFF(x, 1) = 1;
	...
	for (int pos = 0; pos <= GF2_POLY_DEGREE(R); pos++) {
		...
		//               = Omega(a^i) / Denom(a^i)
		GF2_POLY_COEFF(R, pos) = gf2_add(
			GF2_POLY_COEFF(R, pos),
			gf2_div(
				gf2_poly_calc(omega, gf2_pow_a(GF2_MAX_EXP - pos)),
